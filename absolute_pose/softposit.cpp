...
namespace cvg
{
		...
		boost::optional<const CamInfo_type&> maybe_caminfo)
	{
		...
		auto maxDelta = std::sqrt(alpha) / 2;          //  Max allowed error per world point.
		...
		if (0)
		{
			std::cout << "init: " << std::endl << initpose.rot
				<< std::endl << initpose.trans << std::endl;
		...
		while (beta < betaFinal && !assignConverged)
		{
			...
			//std::cout << debug_loop++ << std::endl;

			//此行代码注释掉就会出错，估计是armadilo的原因;
			...
			assignMat(arma::span(0, nbImagePts - 1),
				arma::span(0, nbWorldPts - 1)) = scale * arma::exp(-beta * (distMat - alpha));
			...
			while (poseConverged == false & pose_iter_count < maxCount)
			{
				...
				{
					std::cout
						<< "svd" << std::endl << A << r1 << r2
						<< r3 << Tx << Ty << Tz << std::endl;
				}
				...
				delta = std::sqrt(arma::accu(assignMat.submat(0, 0, nbImagePts - 1, nbWorldPts - 1) % distMat) / nbWorldPts);
			...
			if (0)
			{
				std::cout << "updated pose:" << std::endl <<
					pose.rot << std::endl << pose.trans << std::endl;
	...
	arma::mat sinkhornSlack(arma::mat M)
	{
		...
		while (std::fabs(fMdiffSum) > fEpsilon2 && iNumSinkIter < iMaxIterSinkhorn)
		{
	...
	void softposit(
		float* rot, float* trans, int* foundPose, int* _imagePts,
		float* _worldPts, int nbImagePts, int nbWorldPts,
		float beta0, float noiseStd, float* initRot,
		float* initTrans, float* focalLength, int* center)
	{

		std::vector<point2di_type> imagePts;
		std::vector<point3d_type> worldPts;

		for (int i = 0u; i < nbImagePts; ++i)
		{
			imagePts.push_back(point2di_type{ _imagePts[i * 2], _imagePts[i * 2 + 1] });
		}

		for (int i = 0u; i < nbWorldPts; ++i)
		{
